# modified from https://github.com/JingyunLiang/SwinIR/blob/main/main_test_swinir.py
...
def main():
    ...
    # new: save_dir
    parser.add_argument('--save_dir', type=str, default=None, help='folder to save the output results')
    parser.add_argument('--tile', type=int, default=None, help='Tile size, None for no tile during testing (testing as a whole)')
    parser.add_argument('--tile_overlap', type=int, default=32, help='Overlapping of different tiles')
    args = parser.parse_args()

    # new: check task
    assert args.task in ['real_sr', 'color_dn', 'color_jpeg_car'], "task should be real_sr, color_dn or color_jpeg_car"
...
def setup(args):
    ...
    return args.folder_lq, args.save_dir, border, window_size
...
def get_image_pair(args, path):
    ...
    imgname = f"output_{args.model_path[args.model_path.rfind('_')+1:-4]}"
